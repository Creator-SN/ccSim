...
class AutoModel(IModel):
    ...
    def load_model(self, model_name):
        ...
        if model_name == 'bert':
            self.model = Bert(tokenizer=self.tokenizer, config_path=bert_config_path, pre_trained_path=bert_pre_trained_path)
        elif model_name == 'albert':
            self.model = Albert(tokenizer=self.tokenizer, config_path=albert_config_path, pre_trained_path=albert_pre_trained_path)
        elif model_name == 'roberta':
            self.model = Roberta(tokenizer=self.tokenizer, config_path=roberta_config_path, pre_trained_path=roberta_pre_trained_path)
        elif model_name == 'xlnet':
            self.model = XLNet(tokenizer=self.tokenizer, config_path=xlnet_config_path, pre_trained_path=xlnet_pre_trained_path)
        elif model_name == 'wssbert':
            self.model = WSSBert(tokenizer=self.tokenizer, config_path=bert_config_path, pre_trained_path=bert_pre_trained_path)
        elif model_name == 'esim':
            self.model = ESIM()
        elif model_name == 'bimpm':
            self.model = BIMPM()
        elif model_name == 'sbert':
            self.model = SBert(tokenizer=self.tokenizer, config_path=bert_config_path, pre_trained_path=bert_pre_trained_path)
        elif model_name == 'abcnn':
            self.model = ABCNN()
        elif model_name == 'siagru':
            self.model = SiaGRU()
        elif model_name == 'x':
            self.model = XSSBert(tokenizer=self.tokenizer, config_path=bert_config_path, pre_trained_path=bert_pre_trained_path)
        elif model_name == 'x_k':
