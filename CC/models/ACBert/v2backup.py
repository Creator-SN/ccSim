...
class ACBert(nn.Module):
    ...
    def forward(self, **args):
        ...
        if 'labels' in args:
            loss = mse_loss(cos_sim, args['labels'].view(-1)) + bce_loss(pred, args['labels'].float().view(-1)) + 0.2 * mse_loss(word_cos_sim, args['labels'].view(-1) * 0.2)
        else:
            loss = 0
