    ...
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers):
            if i in self.add_layers:
                total_layers.append(ErnieLayer(config, True))
            else:
                total_layers.append(ErnieLayer(config, False))
...
class ACErnieModel(ErniePreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        super(ACErnieModel, self).__init__(config)
